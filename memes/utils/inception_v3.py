...
def InceptionV3(include_top=True, weights='imagenet',
    ...
    return model, K.get_session().graph
...
def main(GEN_PATH):
    ...
    for meme in os.listdir(GEN_PATH):
        img_path = os.path.join(GEN_PATH, meme, '{}.jpg'.format(meme))
        if os.path.exists(img_path):
            ...
            # filename = os.path.join(GEN_PATH, meme, '{}_img_embedding.npy'.format(meme))
            # np.save(filename, preds)
            # print('Successfully saved prediction in:', filename)

    pred_stacked = np.stack(pred_array, axis=0)
    filename = os.path.join(GEN_PATH, 'img_embeddings.npy')
    np.save(filename, pred_stacked)
    print('Successfully saved prediction in:', filename)
